...
function groupTeamSchedules(schedules: Schedule[]): Schedule[] {
  ...
  const grouped = Array.from(groups.values()).map((items) => {
    ...
    items.forEach((item) => {
      ...
      const names = item.memberIds?.length ? item.memberIds : [item.ownerId]
      titleMap.set(title, Array.from(new Set([...(titleMap.get(title) || []), ...names])))
    })
    const displayTitle = Array.from(titleMap.entries()).map(([title, ids]) => `${ids.join('·')} - ${title}`).join(' / ')
    const base = items[0]
    return { ...base, id: `${base.id}@group`, displayTitle, memberIds: Array.from(new Set(items.flatMap((item) => item.memberIds?.length ? item.memberIds : [item.ownerId]))), description: items.map((item) => item.description).filter(Boolean).join(' / ') }
  })
...
export default function App() {
  ...
  function completeProject(id: string) {
    ...
    maybeNotifySlack('update', completed)
  }
