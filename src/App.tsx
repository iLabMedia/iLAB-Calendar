...
type AppData = { teams: Team[]; staff: Staff[]; schedules: Schedule[]; companyDocs: CompanyDoc[]; slack: SlackSetting; settings: AppSetting }
...
function isNoticeSchedule(schedule: Schedule, teams: Team[] = []) { return schedule.type !== 'project' && teamSortLabel(schedule.teamId, teams) === '공지' }
function projectEndDate(project: Schedule) { return project.repeatUntil || project.date }
function isProjectPastOrCompleted(project: Schedule, todayIso = toISODate(new Date())) { return Boolean(project.completed || projectEndDate(project) < todayIso) }
...
export default function App() {
  ...
  const [companyCategory, setCompanyCategory] = useState<CompanyDocCategory | '전체'>('전체')
  const [companySearch, setCompanySearch] = useState('')
  const [companyDraft, setCompanyDraft] = useState<CompanyDoc>(() => emptyCompanyDoc(currentUserId))
  const [companyNotifySlack, setCompanyNotifySlack] = useState(false)
  const [editingCompanyDocId, setEditingCompanyDocId] = useState('')
  ...
  const announcementSchedules = useMemo(() => data.schedules.filter((s) => isNoticeSchedule(s, data.teams)).sort(sortSchedulesByTeams(data.teams)), [data.schedules, data.teams])
  const normalSchedules = useMemo(() => data.schedules.filter((s) => s.type !== 'project' && !isNoticeSchedule(s, data.teams)), [data.schedules, data.teams])
  const visibleOccurrences = useMemo(() => groupTeamSchedules(normalSchedules.flatMap((s) => getScheduleOccurrences(s, rangeStart, rangeEnd)), data.teams), [normalSchedules, data.teams, rangeStart, rangeEnd])
  const projectSchedules = data.schedules.filter((s) => s.type === 'project').sort((a, b) => a.date.localeCompare(b.date) || projectEndDate(a).localeCompare(projectEndDate(b)) || a.title.localeCompare(b.title))
  const activeProjectSchedules = projectSchedules.filter((project) => !isProjectPastOrCompleted(project, today))
  const mainProjectSchedules = activeProjectSchedules
  const monthStart = toISODate(monthDays[0])
  const monthEnd = toISODate(monthDays[41])
  const monthOccurrences = useMemo(() => groupTeamSchedules(normalSchedules.flatMap((s) => getScheduleOccurrences(s, monthStart, monthEnd)), data.teams), [normalSchedules, data.teams, monthStart, monthEnd])
  const selectedSchedules = useMemo(() => groupTeamSchedules(normalSchedules.flatMap((s) => getScheduleOccurrences(s, selectedDate, selectedDate)), data.teams), [normalSchedules, data.teams, selectedDate])
  const todaySchedules = useMemo(() => groupTeamSchedules(normalSchedules.flatMap((s) => getScheduleOccurrences(s, today, today)), data.teams), [normalSchedules, data.teams, today])
  const dayPopupSchedules = dayPopupDate ? groupTeamSchedules(normalSchedules.flatMap((s) => getScheduleOccurrences(s, dayPopupDate, dayPopupDate)), data.teams) : []
  const filteredCompanyDocs = useMemo(() => data.companyDocs.filter((doc) => {
    if (!isAdmin && !doc.isPublished) return false
    if (companyCategory !== '전체' && doc.category !== companyCategory) return false
  ...
  function editCompanyDoc(doc: CompanyDoc) { setEditingCompanyDocId(doc.id); setCompanyDraft({ ...doc }); setCompanyNotifySlack(false) }
  function resetCompanyDocForm() { setEditingCompanyDocId(''); setCompanyDraft(emptyCompanyDoc(currentUserId)); setCompanyNotifySlack(false) }
  function saveCompanyDoc(event: FormEvent) {
    event.preventDefault()
    ...
    setNotice(companyNotifySlack ? (isNew ? '회사정보가 등록되었고 Slack 알림을 발송합니다.' : '회사정보가 수정되었고 Slack 알림을 발송합니다.') : (isNew ? '회사정보를 등록했습니다.' : '회사정보를 수정했습니다.'))
    if (companyNotifySlack) {
  ...
  function removeCompanyDoc(id: string) {
    if (!confirm('회사정보 문서를 삭제할까요?')) return
    setData((p) => ({ ...p, companyDocs: p.companyDocs.filter((doc) => doc.id !== id) }))
    deleteCompanyDocFromSupabase(id).catch(syncError)
    if (editingCompanyDocId === id) resetCompanyDocForm()
    setNotice('회사정보를 삭제했습니다.')
  }
  ...
  return <main className="appShell">
    ...
    {dayPopupDate && <DayScheduleModal date={dayPopupDate} schedules={dayPopupSchedules} scheduleTitle={scheduleTitle} dateLabel={dateWithWeekday} displayColor={displayColor} onClose={() => setDayPopupDate('')} onAdd={() => openNewSchedule(dayPopupDate)} onEdit={openEdit} />}
...
function AnnouncementTicker({ announcements, scheduleTitle, dateLabel, onEdit }: { announcements: Schedule[]; scheduleTitle: (schedule: Schedule) => string; dateLabel: (iso: string) => string; onEdit: (schedule: Schedule) => void }) {
...
function CompanyInfo({ docs, isAdmin, category, setCategory, search, setSearch, draft, setDraft, editingId, onSubmit, onEdit, onDelete, onCancel, staffName, notifySlack, setNotifySlack }: { docs: CompanyDoc[]; isAdmin: boolean; category: CompanyDocCategory | '전체'; setCategory: (category: CompanyDocCategory | '전체') => void; search: string; setSearch: (value: string) => void; draft: CompanyDoc; setDraft: (doc: CompanyDoc) => void; editingId: string; onSubmit: (event: FormEvent) => void; onEdit: (doc: CompanyDoc) => void; onDelete: (id: string) => void; onCancel: () => void; staffName: (id: string) => string; notifySlack: boolean; setNotifySlack: (value: boolean) => void }) {
...
function ProjectMiniList({ projects, teamName, dateLabel, onEdit }: { projects: Schedule[]; teamName: (id: string) => string; dateLabel: (iso: string) => string; onEdit: (schedule: Schedule) => void }) {
  if (!projects.length) return <div className="emptyState compact">진행 중인 프로젝트가 없습니다.</div>
  return <div className="projectMiniList">{projects.map((project) => <button key={project.id} onClick={() => onEdit(project)} style={{ '--event-color': project.color } as CSSProperties}><b>{project.title}</b><span>{teamName(project.teamId)} · {dateLabel(project.date)}~{dateLabel(project.repeatUntil || project.date)}</span>{project.description && <small>{project.description}</small>}</button>)}</div>
}

function ProjectCalendar({ monthDays, cursor, projects, teamName, dateLabel, onEdit, onPrev, onNext }: { monthDays: Date[]; cursor: Date; projects: Schedule[]; teamName: (id: string) => string; dateLabel: (iso: string) => string; onEdit: (schedule: Schedule) => void; onPrev: () => void; onNext: () => void }) {
