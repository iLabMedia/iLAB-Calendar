...
function groupTeamSchedules(schedules: Schedule[]): Schedule[] {
  ...
  schedules.forEach((schedule) => {
    const key = [schedule.date, schedule.teamId, schedule.title.trim(), schedule.startTime, schedule.endTime, schedule.allDay].join('|')
    const found = groups.get(key)
    if (!found) groups.set(key, { ...schedule, memberIds: schedule.memberIds?.length ? schedule.memberIds : [schedule.ownerId] })
    else groups.set(key, { ...found, memberIds: Array.from(new Set([...(found.memberIds || []), ...(schedule.memberIds?.length ? schedule.memberIds : [schedule.ownerId])])) })
  })
  return Array.from(groups.values()).sort(sortSchedules)
}
...
export default function App() {
  ...
  function canEdit(schedule: Schedule) { if (!currentUser) return false; const s = data.schedules.find((item) => item.id === originalId(schedule.id)) || schedule; return isAdmin || s.teamId === currentUser.teamId || s.createdBy === currentUser.id || s.ownerId === currentUser.id }
